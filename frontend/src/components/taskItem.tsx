...
export const TaskItem = ({ task, onToggle, onDelete, onUpdate, formatDate }: TaskItemProps) => {
    ...
    return (
        <div className={`group flex items-center gap-4 p-5 rounded-2xl border-2 transition-all duration-200 shadow-sm ${task.status === 'concluida'
            ...
            <button
                ...
                className={`shrink-0 transition-colors ${task.status === 'concluida'
                    ? 'text-green-500'
            ...
            <div className="flex-1 flex flex-col">
                {isEditing ? (
                    <input
                        ...
                        className="bg-slate-50 dark:bg-slate-800 border border-blue-300 dark:border-blue-500 rounded-lg px-2 py-1 text-lg font-semibold outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-800 dark:text-slate-100"
                    />
                ) : (
